...
function GaussianVI(m, C, func_Phi, N_iter=200)
    N_x = size(m, 1)
    forward(x) = func_Phi(x), ForwardDiff.gradient(func_Phi, x), ForwardDiff.hessian(func_Phi, x)
    x_mean = zeros(1, N_x)
    xx_cov = zeros(1, N_x, N_x)
    x_w = [1.0]
    T = 0.5 * N_iter
    x_mean[1,:] = m
    xx_cov[1,:,:] = C
    obj = GMNVI_Run(forward, T, N_iter, x_w, x_mean, xx_cov)
    mean = obj.x_mean[end][1,:]
    cov = obj.xx_cov[end][1,:,:]
    return mean, cov
end
