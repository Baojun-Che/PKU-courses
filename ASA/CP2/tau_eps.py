...
if __name__ == "__main__":
    ...
    # 保存数据到 data.txt 文件
    with open('data/eps-T-data.txt', 'w') as file:
        # 写入表头
    ...
    data = np.loadtxt("data/eps-T-data.txt", skiprows=1)
    epsilons = data[:,0]
    T_values = data[:,1]
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('figs/eps-T.pdf')  # 保存图像
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('figs/loglog-eps-T.pdf')  # 保存图像
    plt.show()
